...
#define PLL0_SETUP            1			   //是否连接PLL0
#define PLL0CFG_Val           0x0000000F   //pll0的倍频与分频 M=16 N=1	 275 MHz to 550 MHz
#define PLL1_SETUP            1			   //是否连接PLL1
#define PLL1CFG_Val           0x00000023   //pll1的倍频 M=3 P=1
#define CCLKCFG_Val           0x00000003   //pll的分频	156 MHz to 320 MHz
...
void SystemInit (void)
{
  ...
  LPC_SC->CCLKCFG   = CCLKCFG_Val;      /* Setup Clock Divider   4分频             */

#if (PLL0_SETUP)
  LPC_SC->CLKSRCSEL = CLKSRCSEL_Val;    /* Select Clock Source for PLL0   外部晶振    */
  LPC_SC->PLL0CFG   = PLL0CFG_Val;		 //倍频值为m-1 B：11  N=1 M=16	  (外部晶振*2*M/N)/divide =主频率
...
#if (PLL1_SETUP)
  LPC_SC->PLL1CFG   = PLL1CFG_Val;		//M=4 P=1
  ...
  LPC_SC->PLL1FEED  = 0x55;
#else
  LPC_SC->USBCLKCFG = USBCLKCFG_Val;     /* Setup USB Clock Divider   1         */
#endif
